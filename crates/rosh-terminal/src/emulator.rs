...
use crate::{TerminalError, parser::Parser, framebuffer::{FrameBuffer, Cell}};
...
impl Terminal {
    ...
    pub fn resize(&mut self, width: u16, height: u16) -> Result<(), TerminalError> {
        if width == 0 || height == 0 {
            return Err(TerminalError::SizeError("Invalid terminal size".to_string()));
        }
    ...
    pub fn framebuffer_mut(&mut self) -> &mut FrameBuffer {
        if self.use_alternate {
            self.alternate_buffer.as_mut().unwrap_or(&mut self.framebuffer)
        } else {
    ...
    pub fn restore(&mut self, snapshot: &TerminalSnapshot) -> Result<(), TerminalError> {
        // Resize if needed
        if snapshot.width != self.framebuffer.width() || snapshot.height != self.framebuffer.height() {
        ...
        // Restore cursor
        self.framebuffer.set_cursor_position(snapshot.cursor_x, snapshot.cursor_y);
