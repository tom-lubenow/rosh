    ...
    async fn test_multiline_output() {
        ...
        timeout(Duration::from_secs(2), async {
            while let Some(event) = events.recv().await {
                match event {
                    SessionEvent::StateChanged(state) => {
                        final_output = String::from_utf8_lossy(&state.screen).to_string();
                    }
        ...
        assert!(final_output.contains("Line 1"), "Should see first line");
        assert!(final_output.contains("Line 2"), "Should see second line");
        assert!(final_output.contains("Line 3"), "Should see third line");
        assert!(
            final_output.contains("No newline at end"),
            "Should see text without newline"
        );
    }
}
